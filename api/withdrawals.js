  ...
  try {
    ...
    if (action === 'create' && req.method === 'POST') {
      ...
      const result = await rpc('request_withdrawal', {
        ...
        p_amount: positiveInt(input.amount, 'Amount'),
    ...
    if (action === 'review' && req.method === 'PATCH' && profile.role === 'admin') {
      const input = await body(req);
      const result = await rpc('review_withdrawal', {
        p_admin_id: profile.id,
        p_withdrawal_id: requiredString(input.withdrawal_id, 'Withdrawal ID', 80),
        p_status: oneOf(input.status, 'Status', ['approved', 'rejected', 'processing', 'paid']),
        p_reason: input.reason || null
      });
      return ok(res, { withdrawal: result });
    }

    return fail(res, 405, 'Withdrawal action not supported.');
  } catch (error) {
    return fail(res, error.status || 400, error.message || 'Withdrawal request failed.');
  }
}
