import { apiRequest, isConfigured, publicConfig, ApiError } from './api.js';
import { landing } from './landing.js';

let interests = [];
let interestsLoaded = false;
async function ensureInterestsLoaded() {
  if (interestsLoaded) return interests;
  try {
    const result = await apiRequest('auth?action=interests');
    interests = result.interests || [];
  } catch { interests = []; }
  interestsLoaded = true;
  if (['register', 'profile'].includes(state.activeView)) render();
  return interests;
}
ensureInterestsLoaded();
...
const state = { user: null, role: 'earner', activeView: pageView || 'overview', loading: false, data: {} };
...
function pagePath(view) { const prefix = state.role === 'admin' ? '/admin/' : state.role === 'advertiser' ? '/advertiser/' : '/earner/'; const pages = { overview:'index.html', tasks:'tasks.html', wallet:'wallet.html', referrals:'referrals.html', leaderboard:'leaderboard.html', achievements:'achievements.html', reputation:'reputation.html', announcements:'announcements.html', notifications:'notifications.html', support:'support.html', profile:'profile.html', campaigns:'campaigns.html', analytics:'analytics.html', users:'users.html', withdrawals:'withdrawals.html', settings:'settings.html' }; return prefix + (pages[view] || 'index.html'); }
function sidebar() { const items = state.role === 'admin' ? [['overview','⌂','Overview'],['users','◉','Users'],['campaigns','◌','Campaigns'],['withdrawals','₦','Withdrawals'],['settings','⚙','Settings']] : state.role === 'advertiser' ? [['overview','⌂','Overview'],['campaigns','◌','Campaigns'],['wallet','₦','Wallet'],['analytics','▥','Analytics'],['support','?','Support']] : [['overview','⌂','Overview'],['tasks','✓','Tasks'],['wallet','₦','Wallet'],['referrals','↗','Referrals'],['leaderboard','★','Leaderboard'],['achievements','◆','Achievements'],['reputation','◇','Reputation'],['announcements','▣','Announcements'],['support','?','Support'],['profile','○','Profile']]; return `<aside class="sidebar"><a class="brand" href="${pagePath('overview')}" data-view="overview"><span class="brand-mark">Y</span> YOLOTASK</a><nav>${items.map(([id,icon,label]) => `<a href="${pagePath(id)}" data-view="${id}" class="${state.activeView === id ? 'active' : ''}"><span>${icon}</span>${label}</a>`).join('')}</nav><div class="sidebar-bottom"><a href="${pagePath('settings')}" data-view="settings" style="display:block;padding:10px 14px;font-size:.85rem">⚙ Settings</a><button class="btn btn-ghost" style="width:100%;justify-content:flex-start;margin-top:4px;color:#a8bad1" data-action="logout">↪ Sign out</button></div></aside>`; }
function header() { const notificationUrl = pagePath(state.role === 'admin' ? 'announcements' : 'notifications'); return `<header class="dashboard-header"><button class="btn btn-secondary btn-small mobile-menu" data-action="toggle-sidebar">☰</button><div><strong>${state.activeView === 'overview' ? 'Workspace' : state.activeView[0].toUpperCase()+state.activeView.slice(1)}</strong><span class="muted" style="margin-left:10px;font-size:.8rem">${new Date().toLocaleDateString('en-NG',{weekday:'long',month:'short',day:'numeric'})}</span></div><div class="user-chip"><a class="btn btn-secondary btn-small" href="${notificationUrl}" data-view="${state.role === 'admin' ? 'announcements' : 'notifications'}">◌ <span class="notification-count">0</span></a><div class="avatar">${initials(state.user?.full_name || state.user?.email || 'YO')}</div><span>${esc(state.user?.full_name || 'Member')}</span></div></header>`; }
...
function auth(mode='login') { const register = mode === 'register'; return `<div class="auth-page"><section class="auth-art"><a class="brand" href="#"><span class="brand-mark">Y</span> YOLOTASK</a><div><div class="section-kicker">${register ? 'Join the movement' : 'Welcome back'}</div><h1>${register ? 'Make your next move count.' : 'Good to see you again.'}</h1><p>${register ? 'Create an earner account or join as an advertiser. You choose the lane; YOLOTASK gives you the momentum.' : 'Sign in to see your tasks, wallet, campaigns, and progress.'}</p></div><p style="font-size:.8rem">A Nifelux platform for trusted digital promotion.</p></section><section class="auth-form-wrap"><form class="auth-form" id="auth-form"><a href="#" data-action="home" class="muted" style="display:inline-block;margin-bottom:28px">← Back to YOLOTASK</a><h2>${register ? 'Create your account' : 'Log in'}</h2><p>${register ? 'Start with the essentials. You can refine your profile later.' : 'Use the email and password attached to your account.'}</p><div id="auth-error"></div>${register ? '<label style="margin-bottom:14px">Account type<select name="role"><option value="earner">Earner — complete tasks</option><option value="advertiser">Advertiser — launch campaigns</option></select></label>' : ''}<label style="margin-bottom:14px">Email<input type="email" name="email" autocomplete="email" required placeholder="you@example.com"></label>${register ? '<label style="margin-bottom:14px">Full name<input name="full_name" required placeholder="Your full name"></label>' : ''}<label style="margin-bottom:14px">Password<input type="password" name="password" minlength="8" autocomplete="current-password" required placeholder="At least 8 characters"></label>${register ? `<label style="margin-bottom:14px">Gender<select name="gender"><option value="prefer_not_to_say">Prefer not to say</option><option value="male">Male</option><option value="female">Female</option></select></label><label style="margin-bottom:9px">Select at least 3 interests</label><div class="interest-grid" style="margin-bottom:20px">${interestGrid()}</div>` : ''}<button class="btn btn-primary" style="width:100%" type="submit">${register ? 'Create account ↗' : 'Continue ↗'}</button><p style="text-align:center;margin-top:20px;font-size:.85rem">${register ? 'Already have an account?' : 'New to YOLOTASK?'} <a href="#" data-action="${register ? 'login' : 'register'}" style="color:var(--teal-dark);font-weight:700">${register ? 'Log in' : 'Create one'}</a></p></form></section></div>`; }

function render() { const protectedPage = pageView && !['landing','login','register'].includes(pageView); app.innerHTML = protectedPage ? (state.user ? appView() : auth('login')) : landing(); bindEvents(); }
